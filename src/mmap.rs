...
use c_types::*;
use errno::{set_errno, EINVAL, ENOMEM, EPERM};
use platform::mman::*;
...
pub unsafe extern "C" fn __mmap(
    ...
    if ret == -EPERM as usize && start.is_null() && (flags & MAP_ANON) != 0
        && (flags & MAP_FIXED) == 0
    {
        ret = -ENOMEM as usize;
    }
...
#[linkage = "weak"]
#[no_mangle]
pub unsafe extern "C" fn munmap(start: *mut c_void, len: size_t) -> c_int {
    __munmap(start, len)
}
